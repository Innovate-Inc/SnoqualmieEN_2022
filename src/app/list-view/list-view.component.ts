...
export class ListViewComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    this.selection = new SelectionModel<Element>(
      allowMultiSelect,
      initialSelection
    );
  }
      ...
      .pipe(
        ...
        switchMap((params: ParamMap) => {
          ...
          const relevantKeys = params.keys.filter(
            (key) =>
              !["page", "page_size", "ordering", "table_visible"].includes(key)
          );
      ...
      .pipe(
        tap(() => {
          this.updateQueryParams({
            ...
            start: this.projectService.filter.start,
            num: this.projectService.filter.num,
          });
  ...
  applyQueryParams(params: ParamMap) {
    if (params.get("spatialSelect")) {
      this.spatialSelect = params.get("spatialSelect");
    }
  ...
  search() {
    ...
    const jurisdictionCodes = this.projectService.searchDomainValues(
      this.searchText,
      "Jurisdiction"
    );
    ...
    if (jurisdictionCodes.length > 0) {
      this.projectService.filter.where += ` or Jurisdiction IN (${jurisdictionCodes
        .map((j: string) => `'${j}'`)
        .join(",")})`;
    }
    this.projectService.filter.where += ")";
  ...
  masterToggle(exporter: any) {
    this.isAllSelected()
      ? this.deselectAll(exporter)
      : this.selectAll(exporter);
